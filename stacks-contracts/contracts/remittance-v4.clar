...
(define-public (send-remittance
  ...
  (let (
    ...
    (try! (contract-call? .sbtc-token-v3 transfer amount tx-sender (as-contract tx-sender) none))
...
(define-public (claim-remittance (transfer-id uint) (claim-secret (buff 32)))
  (let (
    ...
    (try! (as-contract (contract-call? .sbtc-token-v3 transfer (get net-amount transfer) tx-sender (get receiver transfer) none)))

    ;; Transfer fee in sBTC from contract escrow to contract owner
    (try! (as-contract (contract-call? .sbtc-token-v3 transfer (get fee transfer) tx-sender CONTRACT-OWNER none)))
...
(define-public (refund-remittance (transfer-id uint))
  (let (
    ...
    (try! (as-contract (contract-call? .sbtc-token-v3 transfer (get amount transfer) tx-sender (get sender transfer) none)))
