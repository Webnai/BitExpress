...
function StatusIcon({ status }: { status: string }) {
  if (status === "claimed")
    return <CheckCircle2 className="w-6 h-6 text-[var(--color-primary)]" />;
  if (status === "pending")
    return <Clock className="w-6 h-6 text-[var(--color-primary)]" />;
  return <XCircle className="w-6 h-6 text-[var(--color-danger-500)]" />;
}

function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    claimed: { label: "Claimed", cls: "bg-[var(--color-primary-soft)] text-[var(--color-primary)]" },
    pending: { label: "Pending", cls: "bg-[var(--color-primary-soft)] text-[var(--color-primary)]" },
    refunded: { label: "Refunded", cls: "bg-[var(--color-surface-muted)] text-[var(--color-text)]" },
  };
  const s = map[status] ?? { label: status, cls: "bg-[var(--color-surface-muted)] text-[var(--color-text-muted)]" };
  return (
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      <div className="mx-auto max-w-[640px] px-4 py-10">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-[var(--color-heading)]">Track Transfer</h1>
        ...
        {/* Search card */}
        <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-[0_4px_18px_rgba(0,0,0,0.22)] p-6 mb-5">
        ...
        {/* Result card */}
        {transaction && (
          <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-[0_4px_18px_rgba(0,0,0,0.22)] p-6 space-y-5">
            {/* Status row */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <StatusIcon status={transaction.status} />
                <div>
                  <p className="text-base font-bold text-[var(--color-heading)]">Transfer Found</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-1 py-3 border-b border-[var(--color-border)]">
                <span className="text-sm text-[var(--color-text-muted)]">Transfer ID</span>
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-mono font-semibold text-[var(--color-heading)]">{transaction.id}</span>
                  <button
                    onClick={copyId}
                    className="text-[var(--color-text-muted)] hover:text-[var(--color-primary)] transition-colors p-0.5"
                  >
            ...
            <div className="flex flex-col gap-2">
              ...
              {transaction.stacksTxId && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full text-center border border-[var(--color-primary)] text-[var(--color-primary)] rounded-lg py-2.5 text-sm font-semibold hover:bg-[var(--color-primary-soft)] transition-colors"
                >
                  Send Tx on Explorer ↗
                </a>
              )}
              ...
              {transaction.refundStacksTxId && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-full text-center border border-[var(--color-border)] text-[var(--color-text)] rounded-lg py-2.5 text-sm font-semibold hover:bg-[var(--color-surface-muted)] transition-colors"
                >
