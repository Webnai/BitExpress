...
const STATUS_CLASS: Record<string, string> = {
  claimed: "bg-[var(--color-primary-soft)] text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)]",
  pending: "bg-[var(--color-primary-soft)] text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)]",
  refunded: "bg-[var(--color-surface-muted)] text-[var(--color-text)] hover:bg-[var(--color-surface-muted)]",
  failed: "bg-[var(--color-danger-soft)] text-[var(--color-danger-500)] hover:bg-[var(--color-danger-soft)]",
};
      ...
      <div className="mx-auto max-w-[1180px] px-4 py-7 md:px-6 md:py-8">
        <section className="grid gap-4 md:grid-cols-3">
          <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-normal text-[var(--color-text-muted)]">Wallet Balance</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="mb-1 text-[2rem] font-bold leading-none text-[var(--color-heading)]">
              ...
              <Badge className="bg-[var(--color-surface-muted)] text-[var(--color-text)] hover:bg-[var(--color-surface-muted)]">On-chain balance</Badge>
            </CardContent>
          </Card>

          <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-normal text-[var(--color-text-muted)]">Transfers In Database</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="mb-1 text-[2rem] font-bold leading-none text-[var(--color-heading)]">{transactions.length}</p>
              <p className="mb-4 text-sm text-[var(--color-text-muted)]">Completed: {completedTransfers}</p>
          ...
          <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-normal text-[var(--color-text-muted)]">Total Volume</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="mb-1 text-[2rem] font-bold leading-none text-[var(--color-heading)]">{formatUsd(totalVolumeUsd)}</p>
              <p className="mb-4 text-sm text-[var(--color-text-muted)]">Derived from wallet history stored in DB</p>
              <Badge className="bg-[var(--color-primary-soft)] text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)]">
        ...
        {error ? (
          <Card className="mt-5 border-[var(--color-danger-500)] bg-[var(--color-danger-soft)] shadow-none">
            <CardContent className="py-4 text-sm text-[var(--color-danger-500)]">{error}</CardContent>
          </Card>
        ) : null}
          ...
          <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
            ...
            <CardContent className="overflow-x-auto pt-0">
              {!isLoading && recentTransactions.length === 0 ? (
                <div className="py-8 text-sm text-[var(--color-text-muted)]">No transfers found for this wallet yet.</div>
              ) : (
                <Table className="min-w-[900px] text-xs">
                  <TableHeader>
                    <TableRow className="border-[var(--color-border)] hover:bg-transparent">
                      <TableHead className="text-[var(--color-text-muted)] font-semibold">Date/Time</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] font-semibold">Transaction ID</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] font-semibold">Direction</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] font-semibold">Amount</TableHead>
                  ...
                  <TableBody>
                    {recentTransactions.map((row) => {
                      ...
                      return (
                        <TableRow key={row.id} className="border-[var(--color-border)] hover:bg-[var(--color-surface-muted)]">
                          <TableCell className="text-[var(--color-text-muted)]">
                            {new Date(row.createdAt).toLocaleString()}
                          </TableCell>
                          <TableCell className="font-medium text-[var(--color-text)]">{shortValue(row.id)}</TableCell>
                          <TableCell>
                            <p className="font-semibold text-[var(--color-heading)]">{displayName}</p>
                            <p className="text-[11px] capitalize text-[var(--color-text-muted)]">{row.direction}</p>
                          </TableCell>
                          <TableCell>
                            <p className="font-semibold text-[var(--color-heading)]">{formatUsd(row.amountUsd)}</p>
                            <p className="text-[11px] text-[var(--color-text-muted)]">Fee {formatUsd(row.fee)}</p>
                          </TableCell>
                          ...
                          <TableCell>
                            {isRefundable ? (
                              <button
                                type="button"
                                className="rounded-md border border-[var(--color-primary)] px-2.5 py-1 text-[11px] font-semibold text-[var(--color-primary)] hover:bg-[var(--color-primary-soft)] disabled:opacity-60"
          ...
          <div className="space-y-5">
            <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
              <CardHeader className="pb-2">
                <CardTitle className="text-xl font-bold text-[var(--color-heading)]">Transfer Volume</CardTitle>
              </CardHeader>
              <CardContent>
                {chartValues.length >= 2 ? (
                  <div className="h-[215px] rounded-xl border border-[var(--color-border)] p-3 bg-[var(--color-surface-muted)]">
                    <svg viewBox="0 0 320 170" className="h-full w-full" role="img" aria-label="transfer volume chart">
                      {[0, 1, 2, 3, 4].map((line) => (
                        <line key={`h-${line}`} x1="0" y1={line * 42.5} x2="320" y2={line * 42.5} stroke="rgba(160,160,160,0.25)" strokeDasharray="3 3" />
                      ))}
                      ...
                      <path d={chartPath} fill="none" stroke="var(--color-primary)" strokeWidth="3" strokeLinecap="round" />
                      {chartValues.map((value, index) => {
                        const min = Math.min(...chartValues);
                        const max = Math.max(...chartValues);
                        const x = (320 / Math.max(chartValues.length - 1, 1)) * index;
                        const y = 170 - ((value - min) / Math.max(max - min, 1)) * (170 - 22) - 11;
                        return <circle key={`${value}-${index}`} cx={x} cy={y} r="3" fill="var(--color-primary)" />;
                      })}
                    </svg>
                  </div>
                ) : (
                  <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-6 text-sm text-[var(--color-text-muted)]">
            ...
            <Card className="border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
              <CardHeader className="pb-2">
                <CardTitle className="text-xl font-bold text-[var(--color-heading)]">Top Countries</CardTitle>
              </CardHeader>
                  ...
                  topCountries.map((country) => (
                    <div key={country.name}>
                      ...
                      <div className="h-2 rounded-full bg-[var(--color-border)]">
                        <div className="h-2 rounded-full bg-[var(--color-primary)]" style={{ width: `${country.percent}%` }} />
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-[var(--color-text-muted)]">No country distribution available yet.</p>
                )}
              </CardContent>
            </Card>
          </div>
        ...
        <Card className="mt-5 border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_4px_18px_rgba(0,0,0,0.22)]">
          <CardHeader className="pb-2">
            <CardTitle className="text-xl font-bold text-[var(--color-heading)]">Recent Activity</CardTitle>
          </CardHeader>
          <Separator className="bg-[var(--color-border)]" />
            ...
            {recentActivity.length ? (
              <div className="grid gap-3 md:grid-cols-4">
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="rounded-xl bg-[var(--color-surface-muted)] p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-[var(--color-primary)]">{formatUsd(activity.amountUsd)}</span>
                      <span className="text-[10px] font-semibold text-[var(--color-text-muted)]">{formatStatus(activity.status)}</span>
                    </div>
                  </div>
                ))}
              </div>
